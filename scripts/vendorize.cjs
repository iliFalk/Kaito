 ...
 *
 * Usage:
 *   node scripts/vendorize.cjs        # performs changes (downloads + overwrites files)
 *   node scripts/vendorize.cjs --dry  # performs a dry-run, prints planned actions but doesn't write files
 *
 * Notes / heuristics:
...
function removeSRIAndCrossorigin(content) {
  // Remove integrity="..." or integrity='...' and crossorigin="..." etc.
...
(async function main() {
  ...
  const allFiles = findFilesRecursive(repoRoot)
    ...
    .filter(f => !f.includes(path.join(repoRoot, 'scripts', 'vendorize.cjs')))
